...
//
// CATATAN PENTING: Jikan API punya rate limit ketat (~3 request/detik, ~60/menit).
// Kalau kena limit, dia balas status 429. Fungsi di bawah otomatis retry dengan jeda
// supaya halaman tidak tampil kosong begitu saja saat traffic lagi ramai.
...
async function jikanFetch(path, retries = 3) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (res.status === 429 && attempt < retries) {
      // Kena rate limit -> tunggu sebentar (makin lama tiap percobaan), lalu coba lagi
  ...
  throw new Error(`Jikan API gagal setelah beberapa percobaan: ${path}`);
}
