...
## 8. Menambahkan chapter manga/manhwa/manhua (fitur baca ala komik)

1. Buka `/admin/dashboard`, scroll ke bagian **"Tambah chapter manga/manhwa/manhua"**.
2. Cari ID MyAnimeList judul yang mau ditambahkan chapternya — bisa dilihat di
   URL halaman detailnya, contoh `/browse/manga/1706` → ID-nya `1706`.
3. Isi nomor chapter, judul chapter (opsional), lalu tempel link gambar tiap
   halaman satu per baris, **urut dari halaman pertama**. Link gambar ini
   harus kamu host sendiri di tempat yang sah (misalnya storage/CDN milikmu,
   bukan hasil scrape situs bajakan).
4. Klik **Simpan chapter**. Member yang punya akses aktif (voucher/gratis)
   bisa langsung membacanya di halaman detail manga tsb, tampil sebagai
   reader vertikal (scroll ke bawah per halaman, mirip baca komik asli),
   lengkap tombol chapter sebelumnya/selanjutnya.
5. Mengulang langkah yang sama dengan nomor chapter yang sama akan
   **menimpa/update** chapter itu (berguna kalau salah upload).
