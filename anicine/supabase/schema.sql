...
-- 6) CHAPTER & HALAMAN MANGA/MANHWA/MANHUA
-- Admin menambahkan chapter + urutan gambar halamannya lewat admin panel.
-- Reader di frontend menampilkan gambar-gambar ini secara vertikal berurutan,
-- seperti membaca komik/manga sungguhan.
create table if not exists manga_chapters (
  id uuid primary key default gen_random_uuid(),
  mal_id int not null,              -- id manga dari Jikan API (MyAnimeList), penghubung ke judul
  chapter_number numeric not null,
  chapter_title text,
  created_at timestamptz not null default now(),
  unique (mal_id, chapter_number)
);

alter table manga_chapters enable row level security;

create policy "semua user login bisa lihat daftar chapter"
...
create policy "semua user login bisa lihat halaman chapter"
  on manga_pages for select
  using (auth.role() = 'authenticated');

-- Insert/update chapter & halaman HANYA lewat service role (admin API).
