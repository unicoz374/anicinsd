  ...
  return (
    <main className="min-h-screen bg-void">
      ...
      <section className="mx-auto max-w-5xl px-6 py-12">
        ...
        {/* TAMBAH CHAPTER MANGA */}
        <div className="mt-6 rounded-lg border border-line bg-panel p-6">
          <h2 className="font-display text-xl text-ink">
            Tambah chapter manga/manhwa/manhua
          </h2>
          <p className="mt-1 font-body text-sm text-muted">
            Isi ID MyAnimeList judulnya (lihat di URL halaman detail, contoh
            .../browse/manga/<b>1706</b>), nomor chapter, lalu tempel link
            gambar tiap halaman (satu baris = satu halaman, berurutan).
          </p>
          <ChapterForm />
        </div>
...
function ChapterForm() {
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch("/api/admin/add-chapter", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ malId, chapterNumber, chapterTitle, imageUrls }),
    });
    ...
    if (!res.ok) {
      setError(data.message || "Gagal menyimpan chapter.");
      return;
    }

    setFeedback(`Chapter ${chapterNumber} tersimpan dengan ${data.totalPages} halaman.`);
    setChapterNumber("");
    setChapterTitle("");
    setImageUrlsText("");
  }
    ...
    <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-3">
      <div className="flex flex-wrap gap-3">
        <input
          ...
          placeholder="ID MyAnimeList (mis. 1706)"
          className="flex-1 min-w-[160px] rounded-lg border border-line bg-panel2 px-4 py-2 font-body text-sm text-ink focus:border-cyan focus:outline-none"
        />
        <input
          value={chapterNumber}
          onChange={(e) => setChapterNumber(e.target.value)}
          placeholder="Nomor chapter (mis. 1)"
          className="w-48 rounded-lg border border-line bg-panel2 px-4 py-2 font-body text-sm text-ink focus:border-cyan focus:outline-none"
        />
        <input
          value={chapterTitle}
      ...
      <textarea
        ...
        placeholder={"Tempel link gambar, satu per baris, urut dari halaman 1:\nhttps://.../halaman-1.jpg\nhttps://.../halaman-2.jpg"}
        rows={6}
        className="rounded-lg border border-line bg-panel2 px-4 py-2 font-body text-sm text-ink focus:border-cyan focus:outline-none"
      />
      <button
        type="submit"
        disabled={loading}
        className="self-start rounded-lg bg-neon px-5 py-2 font-body text-sm text-void hover:bg-ink disabled:opacity-50"
      >
        {loading ? "Menyimpan..." : "Simpan chapter"}
      </button>
      {error && <p className="font-body text-sm text-neon">{error}</p>}
      {feedback && <p className="font-body text-sm text-cyan">{feedback}</p>}
    </form>
  );
}
