...
export default async function HomePage() {
  ...
  let previewManga = [];
  try {
    // Panggil satu-satu (bukan bersamaan) supaya tidak kena rate limit Jikan API
    previewAnime = await getTopAnime(1);
    previewManga = await getTopManga("", 1);
  } catch (e) {
    console.error("Gagal ambil data Jikan API di homepage:", e.message);
  }
